# Архитектурные проблемы модуля io.energy8.identity.UI
...
├── Runtime/
│   ├── Controllers/
│   │   ├── IdentityUIController.cs      # ❌ GOD OBJECT (891 строк)
│   │   ├── IdentityCanvasController.cs  # Canvas управление
│   │   └── GameIdentityUIController.cs  # Наследник
│   ├── Services/
│   │   ├── IdentityService.cs           # Бизнес-логика авторизации
│   │   └── IIdentityService.cs          # Интерфейс сервиса
│   ├── Views/
│   │   ├── Base/
│   │   │   ├── ViewBase.cs              # ❌ Слишком много ответственности
│   │   │   └── IView.cs                 # Базовый интерфейс
│   │   ├── Implementations/             # ❌ Views содержат бизнес-логику
│   │   │   ├── SignInView.cs            # Форма авторизации
│   │   │   ├── UserView.cs              # Профиль пользователя
│   │   │   ├── CodeView.cs              # Ввод кода
│   │   │   └── ... (другие views)
│   │   ├── Management/
│   │   │   ├── ViewManager.cs           # Управление views
│   │   │   ├── ViewFactory.cs           # Фабрика views
│   │   │   └── ViewPresenter.cs         # Презентер
│   │   └── Models/                      # Параметры и результаты views
│   └── Extensions/
│       └── WithLoading.cs               # ❌ Статический coupling
```
