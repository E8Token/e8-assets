...
namespace Energy8.Identity.Core.Auth.Providers
{
    public class WebGLAuthProvider : IAuthProvider
    {
        ...
        {
            logger.LogInfo("Initializing WebGLAuthProvider");
            ...
            try
            {
                ...
                logger.LogInfo("Waiting for auto-authentication with Telegram...");
                ...
                if (hasResult)
                {
                    ...
                    try 
                    {
                        ...
                        logger.LogInfo($"Telegram auto-authentication completed successfully for user: {autoAuthTelegramUser.FirstName} {autoAuthTelegramUser.LastName}");
                    }
                    catch (Exception ex) 
                    {
                        logger.LogError($"Error retrieving Telegram user data: {ex.Message}");
                        autoAuthTelegramUser = null;
                    }
                }
                else
                {
                    logger.LogWarning("Telegram auto-authentication timed out");
                    autoAuthTelegramUser = null;
                }
            ...
            {
                logger.LogError($"Error during Telegram auto-authentication: {ex.Message}");
                IsAutoAuthenticating = false;
                autoAuthInitialized = true;
            }
        }
        ...
        {
            logger.LogInfo("Telegram auto-auth flow completed signal received");
            ...
            if (IsAutoAuthenticating && !autoAuthInitialized && telegramAutoAuthTcs != null)
            {
                if (autoAuthTelegramUser != null)
                {
                    ...
                    logger.LogInfo("Auto-auth completion signal completed the waiting task with actual data");
                }
                else
                {
                    logger.LogWarning("Auto-auth completion signal tried to complete the task, but no user data was available");
                }
            }
        }
        ...
        public async UniTask<TelegramUserDto> SignInWithTelegram(CancellationToken ct)
        {
            ...
            if (autoAuthTelegramUser != null)
            {
                logger.LogInfo($"Using captured Telegram auto-auth data for user: {autoAuthTelegramUser.FirstName} {autoAuthTelegramUser.LastName}");
        ...
        {
            logger.LogInfo($"Received Telegram auth data: {telegramUserJson}");
            ...
            try 
            {
                ...
                if (telegramUserJson.Contains("id=") && telegramUserJson.Contains("hash="))
                {
                    ...
                    logger.LogInfo("Detected query string format, parsing parameters...");
                        ...
                        parameters.TryGetValue("hash", out string hash))
                    {
                        ...
                        if (long.TryParse(idStr, out long id))
                        {
                            ...
                            if (!string.IsNullOrEmpty(authDateStr))
                            {
                                ...
                                logger.LogInfo($"Extracted auth_date: {authDate}");
                            }
                            
                            logger.LogInfo($"Extracted hash: {hash}");
                            ...
                            logger.LogInfo($"Successfully created Telegram user from query params: ID={telegramUser.Id}, Name={telegramUser.FirstName} {telegramUser.LastName}, Username={telegramUser.Username}");
                            telegramAutoAuthTcs.TrySetResult(telegramUser);
                            return;
                        }
                        else
                        {
                            logger.LogError($"Failed to parse user ID: {idStr}");
                            Debug.LogError($"[TelegramAuth] Failed to parse user ID: {idStr}");
                        }
                    }
                    else
                    {
                        logger.LogError("Missing required parameters for Telegram authentication");
                ...
                {
                    logger.LogInfo("Attempting to parse as JSON object");
                    ...
                    try
                    {
                        ...
                        if (DtoBase.TryFromJson(telegramUserJson, out telegramUser))
                        {
                            ...
                            logger.LogInfo($"Successfully parsed Telegram user from JSON: ID={telegramUser.Id}, Name={telegramUser.FirstName} {telegramUser.LastName}, Username={telegramUser.Username}");
                            telegramAutoAuthTcs.TrySetResult(telegramUser);
                            return;
                        }
                        else
                        {
                                ...
                                jObject.TryGetValue("first_name", out var firstNameToken))
                            {
                                ...
                                logger.LogInfo($"Successfully created Telegram user from JObject: ID={telegramUser.Id}, Name={telegramUser.FirstName} {telegramUser.LastName}, Username={telegramUser.Username}");
                                telegramAutoAuthTcs.TrySetResult(telegramUser);
                                return;
                            }
                        }
                    ...
                    {
                        logger.LogError($"Error parsing Telegram JSON data: {ex.Message}");
                        Debug.LogError($"[TelegramAuth] Error parsing JSON: {ex.Message}");
                    }
                }
                
                // If we couldn't parse the data with any method
                logger.LogError($"Failed to parse Telegram user data with any method. Raw data: {telegramUserJson}");
