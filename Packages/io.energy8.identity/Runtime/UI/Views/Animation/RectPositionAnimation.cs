...
namespace Energy8.Identity.Views.Animation
{
    public class RectPositionAnimation : ViewAnimationBase
    {
        ...
        private readonly ILogger<RectPositionAnimation> logger = new Logger<RectPositionAnimation>();
        ...
        {
            IsPlaying = true;
            logger.LogDebug($"Starting position animation from {from} to {to}");
            float elapsed = 0;
            ...
            if (IsPlaying && !ct.IsCancellationRequested)
            {
                ...
                logger.LogDebug("Position animation completed");
            }
        ...
        private void UpdateWidth()
        {
            if (target != null)
            {
                ...
                logger.LogDebug($"Updated width to {desiredWidth} (Screen: {screenWidth}x{screenHeight})");
            }
        }
    }
}